...
class scaleway extends provider implements interfaces\chat_completion, interfaces\create_embedding,
                                           interfaces\create_image, interfaces\vision
{
    private \curl $curl;
    ...
    private static function add_vision_model_field(\MoodleQuickForm $mform, string $element_name_prefix): void
    {
        ...
        $mform->addHelpButton("{$element_name_prefix}vision_model", 'scaleway_vision_model', 'local_mxaimanager');
    }
    ...
    public function vision(string $prompt, array $image_filepaths): vision_request
    {
        if (empty($this->vision_model)) {
            throw new invalid_provider_instance_configuration('Scaleway vision model is not configured');
        }
        ...
        try {
            ...
            if (!isset($json['choices'][0]['message']['content'])) {
                throw new \Exception('Missing content in Scaleway vision response: ' . $response);
            }
